...
// const Vecteur3D sigmaSAMPLE(kB*Temp_ini_x/Zeeman_x, kB*Temp_ini_y/Zeeman_y, sqrt(kB*Temp_ini_z/(2.*Zeeman_z)));
                ...
                {
                    pos.set(j,gsl_ran_gaussian (r,sigma_r));
                    double vi =  gsl_cdf_gaussian_Pinv((1+i)/(Nb_Mol+1.),sigmav.get(j) ); // x^2 = m v^2/(kB T) ; x = v/sigma_v so gaussian distribution is 1/(sqrt(2 pi)) *  exp (-x^2/2)
                    velocity.set(j,  vi  ); // We cut in N+1 interval with equiproba to have a molecule.
...
//    int type_field = params.LocateOrAddNonConstParam("type_field")->val;
    ...
    {
        if(nom_Magn_file !='\0') // At the first initialization we give the name of the file and we have to read the file and initialize the INterpolator
        {
    ...
    {
        if(nom_Elec_file =='\0') // At the first initialization we give the name of the file and we have to read the file and initialize the INterpolator
        {
