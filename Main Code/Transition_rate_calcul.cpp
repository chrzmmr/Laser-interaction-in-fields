...
// TO summarize by default we scan between 0 and Nb_laser. But if we switch it is between 0 and N_laser/2 and then (t>T1) N_laser/2+1 and N_laser
    ...
    {
        int zero_if_t_0_T1_one_if_t_T1_T2 = Is_Switch(params.LocateParam("dt_switch_1")->val, params.LocateParam("dt_switch_2")->val, t); // 0 (DEFAULT) if t is between 0 and T1 (modulo T1+T2); 1 if t is between T1, T1+T2 (modulo T1+T2)
